...
FFMPEG_ACCEL_PRIORITY = ("cuda", "qsv", "videotoolbox", "d3d11va", "dxva2")
...
def resize_images_for_matte(images: list[Image.Image], target_height: int) -> list[Image.Image]:
...
def normalize_matte_mode(raw: str, chroma_enabled: bool) -> str:
    ...
    aliases = {
        ...
        "ai": "birefnet",
        "birefnet": "birefnet",
        "subject": "birefnet_chroma",
        "subject_chroma": "birefnet_chroma",
        "birefnet_chroma": "birefnet_chroma",
        "birefnet+chroma": "birefnet_chroma",
        "birefnet_chroma_key": "birefnet_chroma",
...
def resolve_ffmpeg_binary(name: str) -> str:
    ...
    if fallback_root is not None:
        for suffix in (".exe", ""):
            candidate = fallback_root / f"{name}{suffix}"
            if candidate.exists():
...
def rgb_saturation(rgb: tuple[int, int, int]) -> int:
...
def refine_alpha_mask(
    alpha: Image.Image,
    shrink_pixels: int = 0,
    feather_radius: float = 0.55,
    denoise: bool = True,
    black_point: int = 3,
    white_point: int = 250,
) -> Image.Image:
    mask = alpha.convert("L")
    if denoise and mask.width >= 3 and mask.height >= 3:
        mask = mask.filter(ImageFilter.MedianFilter(3))
    shrink = max(0, int(shrink_pixels or 0))
    if shrink > 0:
        mask = mask.filter(ImageFilter.MinFilter((shrink * 2) + 1))
    if feather_radius > 0:
        mask = mask.filter(ImageFilter.GaussianBlur(radius=float(feather_radius)))

    low = max(0, min(254, int(black_point)))
    high = max(low + 1, min(255, int(white_point)))
    span = high - low
    ...
    def remap(value: int) -> int:
        ...
        if value >= high:
            return 255
        return int(round(smoothstep((value - low) / span) * 255))

    return mask.point(remap)


def pixel_distance(rgb: tuple[int, int, int], key_rgb: tuple[int, int, int]) -> float:
    return math.sqrt(
        (rgb[0] - key_rgb[0]) ** 2
        + (rgb[1] - key_rgb[1]) ** 2
        + (rgb[2] - key_rgb[2]) ** 2
...
def is_key_hue_background_shadow(
    rgb: tuple[int, int, int],
    key_rgb: tuple[int, int, int],
    max_distance: float,
) -> bool:
    if rgb_luma(key_rgb) <= 50 or rgb_saturation(key_rgb) < 20:
        return False
    ...
    other_values = [rgb[index] for index in range(3) if index != dominant]
    key_other_values = [key_rgb[index] for index in range(3) if index != dominant]
    if value < max(other_values) * 1.12:
        return False
    if key_rgb[dominant] < max(key_other_values) * 1.08:
        return False

    distance = pixel_distance(rgb, key_rgb)
    return distance <= max(max_distance * 2.8, 120)


def chroma_key_frame_vectorized(
    image: Image.Image,
    key_rgb: tuple[int, int, int],
    threshold: int,
    softness: int,
    ...
    halo_pixels: int,
    foreground_protect_enabled: bool = False,
    foreground_protect_rgb: tuple[int, int, int] | None = None,
    foreground_protect_tolerance: int = 30,
    foreground_protect_strength: float = 1.0,
) -> Image.Image:
    import numpy as np

    rgba_image = image.convert("RGBA")
    rgba = np.asarray(rgba_image, dtype=np.float32)
    rgb = rgba[..., :3]
    source_alpha = rgba[..., 3]
    key = np.asarray(key_rgb, dtype=np.float32)
    threshold_value = max(0.0, float(threshold))
    softness_value = max(0.0, float(softness))
    max_distance = max(threshold_value + softness_value, 1.0)

    dist = np.sqrt(np.sum((rgb - key) ** 2, axis=2))
    if softness_value <= 0:
        alpha = np.where(dist <= threshold_value, 0.0, 255.0)
    else:
        t = np.clip((dist - threshold_value) / softness_value, 0.0, 1.0)
        smooth = t * t * (3.0 - (2.0 * t))
        alpha = smooth * 255.0

    if rgb_luma(key_rgb) > 50 and rgb_saturation(key_rgb) >= 20:
        dominant = int(np.argmax(key))
        value = rgb[..., dominant]
        other_indices = [index for index in range(3) if index != dominant]
        other_max = np.maximum(rgb[..., other_indices[0]], rgb[..., other_indices[1]])
        key_other_max = max(float(key[other_indices[0]]), float(key[other_indices[1]]))
        shadow = (
            (value >= 34)
            & (value >= other_max * 1.12)
            & (float(key[dominant]) >= key_other_max * 1.08)
            & (dist <= max(max_distance * 2.8, 120.0))
        )
        alpha = np.where(shadow, 0.0, alpha)

    if foreground_protect_enabled and foreground_protect_rgb is not None:
        protect = np.asarray(foreground_protect_rgb, dtype=np.float32)
        protect_tolerance = max(1.0, float(foreground_protect_tolerance or 1))
        protect_strength = max(0.0, min(1.0, float(foreground_protect_strength or 0.0)))
        protect_dist = np.sqrt(np.sum((rgb - protect) ** 2, axis=2))
        protect_t = np.clip(1.0 - (protect_dist / protect_tolerance), 0.0, 1.0)
        protect_weight = protect_t * protect_t * (3.0 - (2.0 * protect_t))
        alpha = np.maximum(alpha, protect_weight * 255.0 * protect_strength)

    alpha = np.minimum(alpha, source_alpha)
    max_rb = np.maximum(rgb[..., 0], rgb[..., 2])
    spill = np.maximum(0.0, rgb[..., 1] - max_rb)
    closeness = np.maximum(0.0, 1.0 - np.minimum(dist / max_distance, 1.0))
    reduction = spill * max(0.0, float(despill_strength or 0.0)) * np.maximum(closeness, 1.0 - (alpha / 255.0))

    output = rgba.copy()
    output[..., 1] = np.maximum(0.0, output[..., 1] - reduction)
    output[..., 3] = alpha
    output_u8 = np.clip(output, 0, 255).astype(np.uint8)
    keyed = Image.fromarray(output_u8, "RGBA")
    alpha_channel = refine_alpha_mask(
        keyed.getchannel("A"),
        shrink_pixels=halo_pixels,
        feather_radius=0.45 if softness > 0 else 0.0,
        denoise=True,
        black_point=2,
        white_point=252,
    )
    keyed.putalpha(alpha_channel)
    return keyed


def edge_connected_background(alpha_values: list[int], width: int, height: int, cutoff: int = 250) -> bytearray:
    connected = bytearray(width * height)
    if width <= 0 or height <= 0:
        return connected

    def can_visit(index: int) -> bool:
        return alpha_values[index] < cutoff and connected[index] == 0

    queue: list[int] = []
    last_row = (height - 1) * width
    for x in range(width):
        for index in (x, last_row + x):
            if can_visit(index):
                connected[index] = 1
                queue.append(index)
    for y in range(height):
        row = y * width
        for index in (row, row + width - 1):
            if can_visit(index):
                connected[index] = 1
                queue.append(index)
...
def preserve_enclosed_key_details(
    ...
    max_bbox_ratio: float = 0.18,
) -> list[int]:
    total = max(1, width * height)
    max_area = max(24, int(total * max_area_ratio))
    max_bbox = max(8, int(min(width, height) * max_bbox_ratio))
    visited = bytearray(width * height)
    output = list(alpha_values)

    for start, value in enumerate(alpha_values):
        if visited[start] or exterior[start] or value > alpha_cutoff:
            continue
        ...
        while stack:
            ...
            for neighbor in (index - 1, index + 1, index - width, index + width):
                ...
                if exterior[neighbor]:
                    touches_exterior = True
                    continue
                if not visited[neighbor] and alpha_values[neighbor] <= alpha_cutoff:
                    visited[neighbor] = 1
        ...
        if (
            component
            and not touches_exterior
            and len(component) <= max_area
            and bbox_width <= max_bbox
            and bbox_height <= max_bbox
        ):
...
def preserve_dark_line_details(
    alpha_values: list[int],
    width: int,
    height: int,
    key_rgb: tuple[int, int, int],
    alpha_cutoff: int = 52,
    foreground_cutoff: int = 150,
    source_alpha_values=None,
) -> list[int]:
    if rgb_luma(key_rgb) > 42:
        return alpha_values
    ...
    for index, alpha in enumerate(alpha_values):
        if source_alpha_values is not None and source_alpha_values[index] <= 0:
            continue
        ...
        for ny in range(max(0, y - radius), min(height, y + radius + 1)):
            ...
            for nx in range(max(0, x - radius), min(width, x + radius + 1)):
                if nx == x and ny == y:
                    continue
                if alpha_values[row + nx] >= foreground_cutoff:
                    foreground_neighbors += 1
        if foreground_neighbors >= min_neighbors:
            output[index] = 255

    return output


def preserve_sandwiched_key_details(
    alpha_values: list[int],
    width: int,
    height: int,
    key_rgb: tuple[int, int, int],
    alpha_cutoff: int = 80,
    foreground_cutoff: int = 140,
    source_alpha_values=None,
) -> list[int]:
    if rgb_luma(key_rgb) > 42:
        return alpha_values
    ...
    def has_foreground_in_direction(x: int, y: int, dx: int, dy: int) -> bool:
        for distance in range(1, search_radius + 1):
            nx = x + (dx * distance)
            ny = y + (dy * distance)
            if nx < 0 or ny < 0 or nx >= width or ny >= height:
                return False
            if alpha_values[(ny * width) + nx] >= foreground_cutoff:
                return True
        return False
    ...
    for index, alpha in enumerate(alpha_values):
        ...
        x = index % width
        y = index // width
        horizontal = has_foreground_in_direction(x, y, -1, 0) and has_foreground_in_direction(x, y, 1, 0)
        vertical = has_foreground_in_direction(x, y, 0, -1) and has_foreground_in_direction(x, y, 0, 1)
...
def preserve_dark_subject_support(
    alpha_values: list[int],
    source_alpha_values: list[int],
    exterior: bytearray,
    width: int,
    height: int,
    key_rgb: tuple[int, int, int],
    support_radius: int = 8,
    alpha_cutoff: int = 110,
    core_cutoff: int = 150,
) -> list[int]:
    if rgb_luma(key_rgb) > 42:
        return alpha_values

    core = Image.new("L", (width, height))
    core.putdata([255 if alpha >= core_cutoff else 0 for alpha in alpha_values])
    size = (max(1, int(support_radius)) * 2) + 1
    support = core.filter(ImageFilter.MaxFilter(size))
    support_values = support.getdata()
    return [
        255 if source_alpha_values[index] > 0 and not exterior[index] and alpha <= alpha_cutoff and support_values[index] > 0 else alpha
        for index, alpha in enumerate(alpha_values)
    ]


def remove_small_key_hue_fragments(
    alpha_values: list[int],
    color_values: list[tuple[int, int, int, int]],
    width: int,
    height: int,
    key_rgb: tuple[int, int, int],
    max_area: int = 260,
    ...
    if rgb_luma(key_rgb) <= 50:
        return alpha_values

    dominant = max(range(3), key=lambda index: key_rgb[index])
    total = width * height
    candidate = bytearray(total)
    for index, (r_value, g_value, b_value, _alpha) in enumerate(color_values):
        if alpha_values[index] <= 0:
            continue
        channels = (r_value, g_value, b_value)
        value = channels[dominant]
        other_values = [channels[item] for item in range(3) if item != dominant]
        same_key_hue = (
            value >= 42
            and value >= max(other_values) * 1.05
            and pixel_distance((r_value, g_value, b_value), key_rgb) <= 180
        )
        if same_key_hue or is_key_hue_background_shadow((r_value, g_value, b_value), key_rgb, 50):
            candidate[index] = 1
    ...
    for start in range(total):
        ...
        for index in component:
            ...
            if transparent_neighbors >= 8:
                break

        should_remove = (
            len(component) <= max_area
            and span <= max_span
            and (transparent_neighbors >= 8 or len(component) >= 120)
        )
        if should_remove:
            for index in component:
                output[index] = 0
...
def neutralize_key_hue_edge_colors(
    color_values: list[tuple[int, int, int, int]],
    alpha_values: list[int],
    width: int,
    height: int,
    ...
    dominant = max(range(3), key=lambda index: key_rgb[index])
    output = list(color_values)
    total = width * height
    for index, (r_value, g_value, b_value, alpha) in enumerate(color_values):
        if alpha_values[index] <= 0:
            continue
        channels = [r_value, g_value, b_value]
        value = channels[dominant]
        other_values = [channels[item] for item in range(3) if item != dominant]
        if value < 42 or value < max(other_values) * 1.05:
            continue
        if pixel_distance((r_value, g_value, b_value), key_rgb) > 180:
            continue
        ...
        channels[dominant] = min(channels[dominant], max(other_values))
        output[index] = (channels[0], channels[1], channels[2], alpha)

    return output


def light_key_background_mask(
    rgba: Image.Image,
    key_rgb: tuple[int, int, int],
    threshold: int,
    softness: int,
    ...
    key_luma = rgb_luma(key_rgb)
    key_saturation = rgb_saturation(key_rgb)
    max_distance = max(threshold + max(softness, 10), threshold + 10)
    visited: set[tuple[int, int]] = set()
    queue: list[tuple[int, int]] = []

    luma_cache: dict[tuple[int, int], float] = {}

    def luma_at(x: int, y: int) -> float:
        key = (x, y)
        cached = luma_cache.get(key)
        if cached is not None:
            return cached
        r_value, g_value, b_value, _ = rgb_pixels[x, y]
        value = rgb_luma((r_value, g_value, b_value))
        luma_cache[key] = value
        return value
    ...
    def local_contrast(x: int, y: int) -> float:
        ...
        for ny in range(max(0, y - 1), min(height, y + 2)):
            for nx in range(max(0, x - 1), min(width, x + 2)):
                if nx == x and ny == y:
                    continue
                strongest = max(strongest, abs(center - luma_at(nx, ny)))
        return strongest

    def is_background_candidate(x: int, y: int) -> bool:
        r_value, g_value, b_value, alpha = rgb_pixels[x, y]
        if alpha <= 0:
            return True
        rgb = (r_value, g_value, b_value)
        luma = rgb_luma(rgb)
        saturation = rgb_saturation(rgb)
        dist = pixel_distance(rgb, key_rgb)

        detail_like = (
            luma < key_luma - 8
            and local_contrast(x, y) >= 16
            and dist > max(8, threshold * 0.22)
        )
        if detail_like:
            return False
        ...
        return (
            ...
            or (
                luma >= key_luma - 18
                and saturation <= max(42, key_saturation + 28)
            )
        )
...
def auto_key_color(image: Image.Image) -> tuple[int, int, int]:
    ...
    for left, top, right, bottom in boxes:
        for y in range(top, bottom):
            for x in range(left, right):
                ...
                buckets.setdefault(bucket, []).append((r_value, g_value, b_value))
    if not buckets:
        return (0, 255, 0)
    ...
    dominant = max(
        ...
        key=lambda values: (
            len(values),
            sum((g - max(r, b)) for r, g, b in values) / max(len(values), 1),
        ),
    )
    return tuple(
        int(sorted(channel_values)[len(channel_values) // 2])
        for channel_values in zip(*dominant)
    )
...
def chroma_key_frame(
    ...
    try:
        return chroma_key_frame_vectorized(
            image=image,
            key_rgb=key_rgb,
            threshold=threshold,
            softness=softness,
            ...
            foreground_protect_enabled=foreground_protect_enabled,
            foreground_protect_rgb=foreground_protect_rgb,
            foreground_protect_tolerance=foreground_protect_tolerance,
            foreground_protect_strength=foreground_protect_strength,
        )
    except Exception:
        pass
    ...
    alpha_values: list[int] = []
    source_alpha_values: list[int] = []
    protected_alpha_values: list[int] = []
    k_r, k_g, k_b = key_rgb
    protect_rgb = foreground_protect_rgb if foreground_protect_enabled else None
    protect_tolerance = max(1, int(foreground_protect_tolerance or 1))
    protect_strength = max(0.0, min(1.0, float(foreground_protect_strength or 0.0)))
    if softness <= 0:
    ...
    for r_value, g_value, b_value, source_alpha in rgba.getdata():
        ...
        shadow_candidate = is_key_hue_background_shadow((r_value, g_value, b_value), key_rgb, max_distance)
        if dist <= threshold or shadow_candidate:
            alpha = 0
        elif softness <= 0 or dist >= max_distance:
            alpha = 255
        else:
            alpha = int(smoothstep((dist - threshold) / softness) * 255)

        protected_alpha = 0
        if protect_rgb is not None and protect_strength > 0:
            protect_dist = pixel_distance((r_value, g_value, b_value), protect_rgb)
            if protect_dist < protect_tolerance:
                protect_weight = smoothstep(1.0 - (protect_dist / protect_tolerance))
                protected_alpha = int(round(255 * protect_weight * protect_strength))
        ...
        alpha_values.append(min(alpha, source_alpha))
        source_alpha_values.append(source_alpha)
        protected_alpha_values.append(protected_alpha)

    exterior = edge_connected_background(alpha_values, rgba.width, rgba.height)
    if rgb_luma(key_rgb) > 50:
        alpha_values = [
            alpha if exterior[index] else 255
            for index, alpha in enumerate(alpha_values)
        ]
    if protect_rgb is not None and protect_strength > 0:
        alpha_values = [
            alpha if exterior[index] else max(alpha, protected_alpha_values[index])
            for index, alpha in enumerate(alpha_values)
        ]
    alpha_values = preserve_enclosed_key_details(alpha_values, rgba.width, rgba.height, exterior)
    alpha_values = preserve_dark_line_details(alpha_values, rgba.width, rgba.height, key_rgb, source_alpha_values=source_alpha_values)
    alpha_values = preserve_dark_subject_support(alpha_values, source_alpha_values, exterior, rgba.width, rgba.height, key_rgb)

    keyed = Image.new("RGBA", rgba.size)
    keyed.putdata(color_pixels)

    alpha_channel = Image.new("L", rgba.size)
    ...
    refined_alpha_values = preserve_dark_line_details(
        list(alpha_channel.getdata()),
        rgba.width,
        rgba.height,
        key_rgb,
        alpha_cutoff=72,
        foreground_cutoff=120,
        source_alpha_values=source_alpha_values,
    )
    refined_alpha_values = preserve_sandwiched_key_details(
    ...
    refined_alpha_values = preserve_dark_subject_support(
        refined_alpha_values,
        source_alpha_values,
        exterior,
        rgba.width,
    ...
    refined_alpha_values = remove_small_key_hue_fragments(
        refined_alpha_values,
        color_pixels,
        rgba.width,
        rgba.height,
    ...
    color_pixels = neutralize_key_hue_edge_colors(
        color_pixels,
        refined_alpha_values,
        rgba.width,
        rgba.height,
    ...
    keyed.putdata(color_pixels)
    alpha_channel = Image.new("L", rgba.size)
    alpha_channel.putdata(refined_alpha_values)
    keyed.putalpha(alpha_channel)
...
def import_corridorkey_dependencies():
    ...
    if not module_dir.exists():
        raise RuntimeError(
            f"CorridorKey is not installed at {root}. Run the setup script or clone {CORRIDORKEY_REPO_URL}."
        )
...
def apply_matte_pipeline(
    ...
    foreground_protect_enabled: bool = False,
    foreground_protect_hex: str = "#2F8F3A",
    foreground_protect_tolerance: int = 34,
    foreground_protect_strength: float = 1.0,
    ...
    try:
        foreground_protect_rgb = parse_hex_color(foreground_protect_hex)
    except Exception:
        foreground_protect_rgb = (47, 143, 58)
    ...
    matte_info = {
        ...
        "foreground_protect_enabled": bool(foreground_protect_enabled),
        "foreground_protect_color": rgb_to_hex(foreground_protect_rgb),
        "foreground_protect_tolerance": max(1, int(foreground_protect_tolerance or 1)),
        "foreground_protect_strength": max(0.0, min(1.0, float(foreground_protect_strength or 0.0))),
    }
    ...
    if mode in {"chroma", "corridorkey"}:
        ...
        for raw_image in raw_images:
            chroma_frame = chroma_key_frame(
                ...
                despill_strength=despill_strength,
                halo_pixels=halo_pixels,
                foreground_protect_enabled=matte_info["foreground_protect_enabled"],
                foreground_protect_rgb=foreground_protect_rgb,
                foreground_protect_tolerance=matte_info["foreground_protect_tolerance"],
                foreground_protect_strength=matte_info["foreground_protect_strength"],
            )
            if use_corridorkey:
    ...
    if mode == "luma":
        ...
        for raw_image in raw_images:
            ...
            alpha = refine_alpha_mask(
                alpha,
                shrink_pixels=matte_info["halo_pixels"],
                feather_radius=0.35,
                denoise=True,
                black_point=1,
                white_point=252,
    ...
    for raw_image in raw_images:
        ...
        if mode in {"birefnet_luma", "birefnet_luma_key", "birefnet_luma_corridorkey"}:
            ...
            alpha = ImageChops.lighter(ai_alpha, chroma_frame.getchannel("A"))
            alpha = refine_alpha_mask(
                alpha,
                shrink_pixels=0,
                feather_radius=0.2,
...
def latest_job_for_upload(upload_id: str, started_after_ms: int = 0) -> dict:
    upload_id = str(upload_id or "").strip()
    if not upload_id:
        raise ValueError("upload id missing")
    min_mtime = max(0.0, float(started_after_ms or 0) / 1000.0 - 2.0)
    ...
    for manifest_path in JOBS_DIR.glob("*/manifest.json"):
        ...
        if manifest.get("upload_id") == upload_id:
            candidates.append((stat.st_mtime, manifest))
    if not candidates:
        raise FileNotFoundError(f"job not found for upload: {upload_id}")
...
def process_video_to_job(
    ...
    matte_images = (
        ...
        else resize_images_for_matte(raw_images, target_size)
    )

    keyed_frames, key_rgb, matte_info = apply_matte_pipeline(
        raw_images=matte_images,
        ...
        foreground_protect_enabled=foreground_protect_enabled,
        foreground_protect_hex=foreground_protect_hex,
        foreground_protect_tolerance=foreground_protect_tolerance,
        foreground_protect_strength=foreground_protect_strength,
    )
        ...
        frame_entries.append(
            {
                ...
                "source_url": f"/work/jobs/{job_id}/raw/{raw_paths[index].name}" if index < len(raw_paths) else "",
        ...
        "frames": [
            {
                "index": 0,
                "name": frame_name,
                "source_url": f"/work/jobs/{job_id}/raw/frame_00001.png",
                "url": f"/work/jobs/{job_id}/processed/{frame_name}",
                "thumb_url": f"/work/jobs/{job_id}/thumbs/{thumb_name}",
            ...
            frame_entries.append(
                {
                    ...
                    "source_url": f"/work/jobs/{job_id}/raw/{frame_name}",
                    "url": f"/work/jobs/{job_id}/processed/{frame_name}",
                    "thumb_url": f"/work/jobs/{job_id}/thumbs/{thumb_name}",
                    "bbox": list(bbox) if bbox else None,
                    "width": image.size[0],
...
def resolve_realesrgan_binary() -> str | None:
    ...
    for path in (
        ...
        WORK_DIR / "tools" / "realesrgan-ncnn-vulkan" / "realesrgan-ncnn-vulkan",
    ):
        if path.exists() and path.is_file():
            return str(path)
    return None
        ...
        try:
            ...
            if parsed.path == "/api/process":
                ...
                result = process_video_to_job(
                    ...
                    foreground_protect_enabled=bool(payload.get("foreground_protect_enabled", False)),
                    foreground_protect_hex=str(payload.get("foreground_protect_hex") or "#2F8F3A"),
                    foreground_protect_tolerance=max(1, safe_int(payload.get("foreground_protect_tolerance"), 34)),
                    foreground_protect_strength=max(0.0, min(1.0, safe_float(payload.get("foreground_protect_strength"), 1.0))),
                ...
                self.send_json({"ok": True, "job": result})
                return
            if parsed.path == "/api/latest-job":
                payload = self.read_json_body()
                result = latest_job_for_upload(
                    upload_id=str(payload.get("upload_id") or ""),
                    started_after_ms=safe_int(payload.get("started_after_ms"), 0),
                )
                self.send_json({"ok": True, "job": result})
                return
